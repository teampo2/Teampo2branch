...
describe('User API Test', function(){
  ...
  it('Create a user - POST', function(done){
    request
      .post('http://localhost:3000/app/logbook')
      .send({
          UserId: randomObjectId(),  //User that owns this log
          AssocId: randomObjectId(),  //Associated object we relate to (optional .e.g. job)
      AddressId: randomObjectId(),  //Address that this log relates to (optional)
          Started: 1,  //When this log entry was started
          Ended: 1,  //When this log entry ended
          DurationMS: 1, //How long was there for in milli seconds
          Activity: "121",  //what we think they were doing
          Confidence: 1, //how confident we are about the activity as a %
          Notes: "213",  //Any notes ?
          Source: "21", //Where did we source the data from
          AvgAccuracy: 1, //Average accuracy in meters
          Updates: 1, //Average accuracy in meters
          Coordinates: {
              xlon: 1,
              ylat: 1
          }
  ...
  it('Get it back again - GET ' + dbid, function(done){
    request
     .get('http://localhost:3000/app/location/:id')
  ...
  */
  ...
  it('Modify a user - PUT '+dbid, function(done){
    request
      .put('http://localhost:3000/app/location/:id')
      .send({
        //isactive:     true,
       // issysadmin:   true
      })
  ...
  it('Delete it - DEL ' + dbid, function (done) {
      request
        .del('http://localhost:3000/app/location/:id')
        .end(function (res) {
            res.statusCode.should.equal(200);
            res.body.success.should.equal(true);
            done();
