...
describe('User API Test', function(){
  ...
  //app.get ('/api/address', function(req,res) { addressController.list(req, res) });       //Get all
  //app.get ('/api/address/:id', function(req,res) { addressController.get(req, res) });    //Get a
  //app.post('/api/address', function(req,res) { addressController.create(req, res) });     //Create a
  //app.put ('/api/address/:id', function(req,res) { addressController.update(req, res) }); //Update a
  //app.del ('/api/address/:id', function(req,res) { addressController.delete(req, res) }); //Delete a
  ...
  it('Create a user - POST', function(done){
    request
      ...
      .send({
        Username:     "te444333st123",
        Password:   "te44s123333t",
        Email:      "te44am331231po2@gmail.com",
      })
      .end(function (res) {
  ...
  */
  ...
  it('Modify a user - PUT '+dbid, function(done){
    request
      .put('http://localhost:3000/app/user/'+dbid)
      .send({
        isactive:     true,
        issysadmin:   true
      })
  ...
  it('Delete it - DEL ' + dbid, function (done) {
      request
        .del('http://localhost:3000/app/user/' + dbid)
        .end(function (res) {
            res.statusCode.should.equal(200);
            res.body.success.should.equal(true);
            done();
